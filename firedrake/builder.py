...
class Director:
    ...
    def getFunctionSpace(self):
        ...
        functionspace = BuiltFunctionSpace()

        element = self.__builder.getElement()
        element = self.__builder.checkElement()
        functionspace.setElement(element)

        imp = self.__builder.getImplementation()
        functionspace.setImplementation(imp)
...
class functionSpace:
    ...
    def checkElement(self, el, top=True):
...
def FunctionSpace(mesh, family, degree=None, name=None, vfamily=None,
                  vdegree=None):
    director = Director()
    director.setBuilder(functionSpaceBuilder)

    function_space_obj = director.getFunctionSpace()
